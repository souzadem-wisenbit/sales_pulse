...
const LiveCoach = (() => {
  ...
  let activeRecorders = [];
  let audioCtx = null;
  let soundFlags = { seller: false, client: false };
  ...
  function baseStyles() {
    return `
      <style>
        ...
        .lc-wrap { max-width: 1100px; margin: 0 auto; padding: 1.5rem; }
        ...
        .lc-grid { display: grid; grid-template-columns: 1fr 360px; gap: 1.25rem; }
        ...
        .lc-transcript { max-height: 60vh; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; }
      ...
      <div class="lc-wrap">
        ...
        <div class="lc-grid">
          <div class="lc-card">
            ...
            <div class="lc-setup-step"><div class="lc-step-num">2</div><div>Clique em <strong>Iniciar</strong> abaixo e selecione a <strong>aba da reunião</strong>, marcando a opção <strong>"Compartilhar áudio da guia"</strong> — é assim que o coach ouve o cliente.</div></div>
            <div class="lc-setup-step"><div class="lc-step-num">3</div><div>Permita o acesso ao <strong>microfone</strong> — é assim que o coach ouve você. Sua fala e a do cliente são identificadas automaticamente.</div></div>
            ...
            <div class="lc-setup-step"><div class="lc-step-num">5</div><div>Volte para a aba da reunião e venda normalmente. As dicas chegam como <strong>notificações do navegador</strong> e ficam registradas aqui.</div></div>
          </div>
  ...
  async function start() {
    ...
    try {
      ...
      tips = [];
      lastCoachedCount = 0;
      soundFlags = { seller: false, client: false };
      activeRecorders = [];

      // Monitor de som (para pular blocos de silêncio) + gravação por canal
  ...
  function monitorSound(stream, speaker) {
    try {
      ...
      const tick = () => {
        ...
        if (Math.sqrt(sum / data.length) > 0.02) soundFlags[speaker] = true;
        setTimeout(tick, 250);
      };
      tick();
  ...
  function startChunkLoop(stream, speaker) {
    const recordChunk = () => {
      ...
      recorder.onstop = () => {
        ...
        const hadSound = soundFlags[speaker];
        soundFlags[speaker] = false;
        const blob = new Blob(chunks, { type: 'audio/webm' });
        if (hadSound && blob.size > 3000) transcribeChunk(blob, speaker);
  ...
  function isNoise(text) {
    ...
    // Alucinações clássicas do Whisper em trechos de quase-silêncio
    const junk = ['amara.org', 'legendas pela comunidade', 'legendado por', 'transcrito por'];
  ...
  async function transcribeChunk(blob, speaker) {
    try {
      ...
      if (!text || text.length < 2 || isNoise(text)) return;
...
Se realmente não houver nada útil a dizer agora, retorne {"tip": null}`;
  ...
  function renderLive() {
    ...
    overlay.innerHTML = `${baseStyles()}
      <div class="lc-wrap">
        ...
        <div class="lc-muted" style="margin-bottom:1rem">Você pode voltar para a aba da reunião — as dicas chegam como notificações do navegador. 🎤 = você · 🖥 = cliente (áudio da aba compartilhada)</div>
        <div class="lc-grid">
          <div class="lc-card">
  ...
  async function stop(skipConfirmClose = false) {
    ...
    try {
      if (transcript.length >= 4) {
        ...
        const prompt = `Você é um coach de vendas sênior. Abaixo está a transcrição de uma chamada de vendas REAL do vendedor "${user?.name || 'Vendedor'}". O canal CLIENTE pode conter mais de uma pessoa do lado remoto — se o contexto permitir distinguir, mencione no resumo.
